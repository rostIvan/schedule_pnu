...
class SelectPage extends StatelessWidget {
  ...
  Widget _buildFab() => StoreConnector<SearchState, ScheduleType>(
      ...
      builder: (context, searchType) => FloatingActionButton(
          ...
          child: searchType == ScheduleType.GROUP ? Icon(Icons.person) : Icon(Icons.group)
      )
  );
  ...
  Widget _buildSearchArea() => StoreConnector<SearchState, SearchState>(
      converter: (store) => store.state,
      builder: (context, state) => state.isSearching ? _progress() : _suggestions(state.results)
  );
...
class SearchTextField extends StatelessWidget {
  ...
  String _hint(searchType) => 'Введіть назву ' + (searchType == ScheduleType.GROUP ? 'групи' : 'викладача');
}
...
class SuggestionListView extends StatelessWidget {
  ...
  Widget _buildItem(int index, String title) => StoreConnector<SearchState, ListItem>(
      converter: (store) => store.state.selectedItem,
      builder: (context, selectedItem) => _buildSuggestion(selectedItem, index, title)
  );

  SuggestionItem _buildSuggestion(ListItem selectedItem, int index, String title) =>
      selectedItem == null || selectedItem.position == null || selectedItem.position != index ?
    SuggestionItem(index, title, onClick: (index, text) => interactor.tapOnItem(ListItem(index, text))) :
    SuggestionItem(index, title, onClick: (index, text) =>
        interactor.tapOnItem(ListItem(index, text)), backgroundColor: Colors.blue, textColor: Colors.white);
}
